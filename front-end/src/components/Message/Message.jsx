...
const Message = ({ onClose, activeBot, setActiveBot }) => {
  ...
  const employeeId = Number(localStorage.getItem("employee_id") || NaN);
  const employerId = Number(localStorage.getItem("employer_id") || NaN);
  ...
  useEffect(() => {
    ...
    const getTeam = async () => {
      try {
        ...
        // Only add the parameter that has a valid value
        if (employeeId && !isNaN(employeeId)) {
          params.employee_id = employeeId;
        } else if (employerId && !isNaN(employerId)) {
          params.employer_id = employerId;
        } else {
          console.warn("Neither valid employeeId nor employerId provided");
          return;
        }

        const res = await axios.get(`${BASE_URL}team`, { params });
        setTeam(res.data.team);
        console.log("Team data:", res.data.team);
      } catch (err) {
    ...
    if (!employeeId) {
      const getName = async () => {
        try {
          ...
          console.log("myname is: get name ", res.data.firs_name);
        } catch (err) {
          console.error(err);
        }
      }
  ...
  const fetchConversations = async () => {
    if (employeeId) {
      try {
        const res = await axios.get(`${BASE_URL}employee/conversation/${userId}`);
      ...
      try {
        const res = await axios.get(`${BASE_URL}employer/conversation/${userId}`);
        setConversations(res.data);
      } catch (err) {
        console.error(err);
      }
  ...
  const createConversation = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        participants,
        name: chatType === "group" ? groupName.trim() : "Direct Chat"
      };
  ...
  const sendMessage = async (text = "") => {
    ...
    try {
      ...
      const tempMessage = {
        ...
        sender_id: userId,
        text,
        attachment_url: previewUrl, // ✅ LOCAL INSTANT PREVIEW
        attachment_type: file?.type || null,
        created_at: new Date().toISOString(),
        pending: true, // ✅ mark as uploading
      };
        ...
        prev.map((msg) =>
          msg.id === tempId
            ? {
              ...
              attachment_url: `${res.data.attachment_url}?t=${Date.now()}`, // ✅ bust cache
  ...
  const conversationNameConvertor = (conv) => {
    if (!conv?.name) return "Conversation";

    if (conv.type === "group") return conv.name;

    const [name1, name2] = conv.name.split("&");

    return name1 === myName ? name2 : name1;
  };
                    ...
                    <div key={index}>
                      ...
                      >
                        <div className="flex justify-between">
                          ...
                          <span>
                            {/* <span>{conversationNameConvertor(conv)}</span> */}
                            {
                              conv.type == "group" ? (
                                <h1>{conv.name}</h1>
                              ) : <h1>{conv.name.split("&")[0]} <span> and </span> {conv.name.split("&")[1]}</h1>
                            }
                          </span>
